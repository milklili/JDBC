...
public class Showteacher implements ActionListener,ListSelectionListener{
JTable table=null;
...
public Showteacher (){
    ...
    addButton=new JButton("增加教师信息");
    panel.add(addButton);
    addButton.addActionListener(this);
    alterButton=new JButton("修改教师信息");
    panel.add(alterButton);
    alterButton.addActionListener(this);
    deleteButton=new JButton("删除教师信息");  
    ...
    if (e.getSource() == deleteButton){
      //selectionMode.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    	...
    	if(table.getSelectedRow()==-1){
    	    ...
    	    int option = JOptionPane.showConfirmDialog(null,"信息删除后无法复原，是否删除？", "删除信息", JOptionPane.YES_NO_OPTION,
    		       JOptionPane.WARNING_MESSAGE, null);
    		     switch (option) {
    		     case JOptionPane.YES_OPTION: {
    		    	 String sql = "delete from teacher where tid='"+currentvalue+"'";
    		     	update(sql);
